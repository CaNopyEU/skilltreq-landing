export default {
  // Meta
  'meta.title': 'SkillTreq — Skill Tracker with Dependency Trees',
  'meta.description':
    'Track your skills as a dependency tree. Log workouts, follow training plans, and see real progress.',

  // Nav
  'nav.skipToContent': 'Skip to content',
  'nav.home': 'Home',
  'nav.privacy': 'Privacy Policy',
  'nav.terms': 'Terms of Service',

  // Hero
  'hero.headline': 'Your skills as a dependency tree.',
  'hero.headline.pre': 'Your skills as a ',
  'hero.headline.accent': 'dependency tree',
  'hero.headline.post': '.',
  'hero.subtitle':
    'Track your skills as a dependency tree. Structured plans, workout logging, and real data — not gamification.',
  'hero.cta': 'Try free',
  'hero.secondary': 'Learn more',

  // Features
  'features.heading': 'Built for athletes who track, not grind.',
  'features.skillTree.title': 'Skill Tree',
  'features.skillTree.description':
    'See your skills as a dependency graph. Know exactly what to train next based on prerequisites you already have.',
  'features.plans.title': 'Training Calendar',
  'features.plans.description':
    "Schedule workouts, follow structured plans, and see what's coming. Your training week at a glance.",
  'features.logging.title': 'Active Workout',
  'features.logging.description':
    'Step-by-step guided sessions with timer, reps, and holds. Just follow along and log as you go.',
  'features.progress.title': 'Progress Tracking',
  'features.progress.description':
    'Activity heatmap, skill stats, and personal records. See your real progress over time — no fake XP bars.',
  'features.coach.title': 'Coach System',
  'features.coach.description':
    'Manage clients, assign plans, and monitor their activity. Built for real coaches, not influencers.',

  // Blog
  'nav.blog': 'Blog',
  'blog.title': 'Blog',
  'blog.description': 'Development updates, guides, and announcements from the SkillTreq team.',
  'blog.readMore': 'Read more',
  'blog.allPosts': 'All posts',
  'blog.noPosts': 'No posts yet. Check back soon.',
  'blog.type.devlog': 'Devlog',
  'blog.type.guide': 'Guide',
  'blog.type.roadmap': 'Roadmap',
  'blog.type.announcement': 'Announcement',
  'blog.tags': 'Tags',
  'blog.publishedOn': 'Published on {date}',
  'blog.by': 'by {author}',
  'blog.linkedChanges': 'What changed',
  'blog.backToList': 'Back to blog',
  'blog.rss': 'RSS Feed',

  // Footer
  'footer.tagline': 'Built for athletes who track, not grind.',
  'footer.beta': 'SkillTreq is free during Beta. Paid plans coming later.',
  'footer.support': 'Support us',
  'footer.copyright': '© {year} SkillTreq. All rights reserved.',

  // Theme
  'theme.toggle': 'Theme',
  'theme.light': 'Light',
  'theme.dark': 'Dark',
  'theme.system': 'System',

  // Language
  'lang.en': 'English',
  'lang.sk': 'Slovenčina',
  'lang.cs': 'Čeština',

  // PWA Install
  'pwa.heading': 'Install as an app',
  'pwa.subtitle':
    'SkillTreq works as a progressive web app. Add it to your home screen for a native app experience — no app store needed.',
  'pwa.ios.title': 'iPhone / iPad',
  'pwa.ios.step1': 'Open skilltreq.app in Safari',
  'pwa.ios.step2': 'Tap the Share button (square with arrow)',
  'pwa.ios.step3': 'Scroll down and tap "Add to Home Screen"',
  'pwa.android.title': 'Android',
  'pwa.android.step1': 'Open skilltreq.app in Chrome',
  'pwa.android.step2': 'Tap the three-dot menu',
  'pwa.android.step3': 'Tap "Install app" or "Add to Home screen"',
  'pwa.desktop.title': 'Desktop',
  'pwa.desktop.step1': 'Open skilltreq.app in Chrome or Edge',
  'pwa.desktop.step2': 'Click the install icon in the address bar',

  // Goal Picker
  'goals.heading': 'What do you want to achieve?',
  'goals.subheading': 'Pick a goal and see your roadmap to get there.',
  'goals.cta.default': 'Try free',
  'goals.cta.selected': 'Start training {goal}',
  'goals.roadmap.title': 'Your path to {goal}',
  'goals.goal.muscle-up': 'Muscle-up',
  'goals.goal.handstand': 'Handstand',
  'goals.goal.front-lever': 'Front Lever',
  'goals.goal.human-flag': 'Human Flag',
  'goals.goal.front-split': 'Front Split',
  'goals.goal.aerial-cartwheel': 'Aerial Cartwheel',

  // Demo libraries
  'demo.library.calisthenics': 'Calisthenics',
  'demo.library.mobility': 'Mobility',
  'demo.library.acrobatics': 'Acrobatics',

  // Calisthenics demo skills
  'demo.skill.ring-support': 'Support',
  'demo.skill.ring-dip': 'Ring Dip',
  'demo.skill.ring-push-up': 'Ring PU',
  'demo.skill.ring-pppu': 'PPPU',
  'demo.skill.tuck-planche': 'Tuck Planche',
  'demo.skill.ring-row': 'Ring Row',
  'demo.skill.ring-pull-up': 'Pull-up',
  'demo.skill.ring-mu': 'Muscle-up',
  // Mobility demo skills
  'demo.skill.90-90-stretch': '90/90',
  'demo.skill.pigeon-pose': 'Pigeon',
  'demo.skill.hip-pails-rails': 'Hip P/R',
  'demo.skill.front-split': 'Front Split',
  'demo.skill.cat-cow': 'Cat-Cow',
  'demo.skill.jefferson-curl': 'Jeff Curl',
  'demo.skill.shoulder-cars': 'Shldr CARs',
  'demo.skill.full-bridge': 'Bridge',
  // Acrobatics demo skills
  'demo.skill.tmb-cartwheel': 'Cartwheel',
  'demo.skill.tmb-one-hand-cw': '1H Cartwheel',
  'demo.skill.tmb-aerial': 'Aerial',
  'demo.skill.tmb-round-off': 'Round-off',
  'demo.skill.tmb-bridge': 'Bridge',
  'demo.skill.tmb-bhs': 'BHS',
  'demo.skill.tmb-back-tuck': 'Backflip',
  // Kickboxing demo skills
  'demo.skill.kb-stance': 'Stance',
  'demo.skill.kb-jab': 'Jab',
  'demo.skill.kb-cross': 'Cross',
  'demo.skill.kb-hook': 'Hook',
  'demo.skill.kb-jch': 'J-C-H',
  'demo.skill.kb-front-kick': 'Front Kick',
  'demo.skill.kb-roundhouse': 'Roundhouse',

  // Demo library
  'demo.library.kickboxing': 'Kickboxing',

  // Goals
  'goals.goal.planche': 'Planche',
  'goals.goal.bridge': 'Bridge',
  'goals.goal.backflip': 'Backflip',
  'goals.goal.roundhouse-kick': 'Roundhouse Kick',
  'goals.goal.jab-cross-hook': 'Jab-Cross-Hook',

  // Library Request
  'libraryRequest.heading': 'Every skill has a path.',
  'libraryRequest.description':
    "We're building skill libraries across movement disciplines. Don't see yours?",
  'libraryRequest.cta': 'Request a library',
  'libraryRequest.stats': '{skills} skills across {libraries} libraries — and growing.',

  // Discipline groups
  'discipline.calisthenics': 'Calisthenics',
  'discipline.rings': 'Rings',
  'discipline.tumbling': 'Tumbling',
  'discipline.tricking': 'Tricking',
  'discipline.mobility': 'Mobility',
  'discipline.kickboxing': 'Kickboxing',
  'discipline.gym': 'Gym / Strength',
  'discipline.juggling': 'Juggling',
  'discipline.flow-arts': 'Flow Arts',
  'discipline.jump-rope': 'Jump Rope',
  'discipline.ukemi': 'Ukemi',
  'discipline.skillCount': '{count} skills',

  // Skill descriptions
  'demo.desc.ring-support': 'Static hold on rings with arms locked out. Foundation for all ring work.',
  'demo.desc.ring-dip': 'Dip on gymnastic rings — significantly harder than parallel bars due to instability.',
  'demo.desc.ring-push-up': 'Push-up with hands on rings. Increased instability demands greater shoulder stabilization.',
  'demo.desc.ring-pppu': 'Pseudo planche push-up on rings. Leaning forward shifts load toward planche position.',
  'demo.desc.tuck-planche': 'Hold body horizontal on rings with knees tucked to chest. First true planche progression.',
  'demo.desc.ring-row': 'Inverted row on rings — pulling body up from an angled hang. Builds foundational pull strength.',
  'demo.desc.ring-pull-up': 'Full pull-up on rings with free rotation. Harder than bar due to stabilization demands.',
  'demo.desc.ring-mu': 'Pull-up transitioning over the rings into a dip. The ultimate upper-body ring skill.',
  'demo.desc.90-90-stretch': 'Both legs at 90° angles targeting hip internal and external rotation simultaneously.',
  'demo.desc.pigeon-pose': 'Deep hip opener with front leg externally rotated and back leg extended. Key for front split.',
  'demo.desc.hip-pails-rails': 'Progressive and regressive isometric loading for hip end-range. Builds active flexibility.',
  'demo.desc.front-split': 'Full front split with hips square. Requires both passive range and active control.',
  'demo.desc.cat-cow': 'Alternating spinal flexion and extension on all fours. Warms up the entire spine segmentally.',
  'demo.desc.jefferson-curl': 'Standing segmental spinal flexion with light load. Targets hamstrings and posterior chain.',
  'demo.desc.shoulder-cars': 'Controlled Articular Rotations for the shoulder. Slow circles through full available range.',
  'demo.desc.full-bridge': 'Full bridge with arms and legs extended, hips pushed high. Requires shoulder and spine mobility.',
  'demo.desc.tmb-cartwheel': 'Lateral rotation through hand-hand-foot-foot pattern with straight legs.',
  'demo.desc.tmb-one-hand-cw': 'Cartwheel with only one hand touching the ground. Builds aerial awareness.',
  'demo.desc.tmb-aerial': 'No-hands cartwheel — a true aerial rotation. The iconic tumbling milestone.',
  'demo.desc.tmb-round-off': 'Cartwheel variant that lands with both feet together facing the start. Entry for flips.',
  'demo.desc.tmb-bridge': 'Full bridge position — hands and feet on ground, hips pushed high. Opens shoulders and spine.',
  'demo.desc.tmb-bhs': 'Jump backwards onto hands, then snap feet over into standing. Gateway to backflips.',
  'demo.desc.tmb-back-tuck': 'Standing backward somersault in tucked position. The quintessential tumbling skill.',
  'demo.desc.kb-stance': 'Balanced guard position with lead foot forward, hands protecting chin and body.',
  'demo.desc.kb-jab': 'Quick straight punch with the lead hand. Sets up combinations and controls distance.',
  'demo.desc.kb-cross': 'Power straight punch from the rear hand with full hip rotation. The knockout punch.',
  'demo.desc.kb-hook': 'Circular punch targeting the side of the head or body. Devastating at close range.',
  'demo.desc.kb-jch': 'The classic three-punch combination. Foundation of all striking combinations.',
  'demo.desc.kb-front-kick': 'Pushing kick with the ball of the foot to the midsection. Creates distance and disrupts rhythm.',
  'demo.desc.kb-roundhouse': 'Powerful rotational kick striking with the shin. The most used kick in kickboxing.',

  // Demo statuses
  'demo.status.locked': 'Locked',
  'demo.status.unlocked': 'Unlocked',
  'demo.status.in_progress': 'In Progress',
  'demo.status.completed': 'Completed',
  'demo.status.mastered': 'Mastered',
  'demo.tooltip.cta': 'Try in app',
} as const;
